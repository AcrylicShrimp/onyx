...
extern "C" VKAPI_ATTR VkBool32 VKAPI_CALL __callbackDebug(
...
namespace onyx::core {
		...
		sExtensionMap{ContextManager::fetchExtension()}
	{
		...
		VkApplicationInfo vkApplicationInfo{VkStructureType::VK_STRUCTURE_TYPE_APPLICATION_INFO,
											nullptr,
											this->pInstance->sApplicationVersion.sName.data(),
											static_cast<std::uint32_t>(VK_MAKE_VERSION(
												this->pInstance->sApplicationVersion.nMajor,
												this->pInstance->sApplicationVersion.nMinor,
												this->pInstance->sApplicationVersion.nPatch)),
											this->pInstance->sVersion.sName.data(),
											static_cast<std::uint32_t>(VK_MAKE_VERSION(
												this->pInstance->sVersion.nMajor,
												this->pInstance->sVersion.nMinor,
												this->pInstance->sVersion.nPatch)),
											VK_API_VERSION_1_0};

		VkInstanceCreateInfo vkInstanceCreateInfo
		{
			VkStructureType::VK_STRUCTURE_TYPE_INSTANCE_CREATE_INFO, nullptr, 0, &vkApplicationInfo,
#if _DEBUG
				nLayerNameCount, vLayerNameArray,
#else
				0, nullptr,
#endif
				nExtensionNameCount, vExtensionNameArray
		};
		...
		auto vkCreateDebugUtilsMessengerEXT{reinterpret_cast<PFN_vkCreateDebugUtilsMessengerEXT>(
			vkGetInstanceProcAddr(this->vkInstance, "vkCreateDebugUtilsMessengerEXT"))};

		if (!vkCreateDebugUtilsMessengerEXT) throw std::runtime_error{"unable to load vkCreateDebugUtilsMessengerEXT"};

		if (vkCreateDebugUtilsMessengerEXT(
				this->vkInstance,
				&vkDebugMessengerCreateInfo,
				nullptr,
				&this->vkDebugMessenger)
			!= VkResult::VK_SUCCESS)
	...
	ContextManager::~ContextManager() noexcept
	{
		...
		auto vkDestroyDebugUtilsMessengerEXT{reinterpret_cast<PFN_vkDestroyDebugUtilsMessengerEXT>(
			vkGetInstanceProcAddr(this->vkInstance, "vkDestroyDebugUtilsMessengerEXT"))};
	...
	std::vector<VkPhysicalDevice> ContextManager::fetchPhysicalDevice() const
	{
		...
		if (vkEnumeratePhysicalDevices(this->vkInstance, &nDeviceCount, sPhysicalDeviceList.data())
			!= VkResult::VK_SUCCESS)
			throw std::runtime_error{"unable to query physical device of vulkan instance"};
	...
	std::unordered_map<std::string, VkLayerProperties> ContextManager::fetchLayer()
	{
		...
		for (const auto &vkLayer: sLayerList)
			std::memcpy(
				&sLayerMap
					 .emplace(
						 std::string{std::cbegin(vkLayer.layerName),
									 std::find(std::cbegin(vkLayer.layerName), std::cend(vkLayer.layerName), '\0')},
						 VkLayerProperties{})
					 .first->second,
				&vkLayer,
	...
	std::unordered_map<std::string, VkExtensionProperties> ContextManager::fetchExtension()
	{
		...
		if (vkEnumerateInstanceExtensionProperties(nullptr, &nExtensionCount, sExtensionList.data())
			!= VkResult::VK_SUCCESS)
			throw std::runtime_error{"unable to query supported extension of vulkan"};
		...
		for (const auto &vkExtension: sExtensionList)
			std::memcpy(
				&sExtensionMap
					 .emplace(
						 std::string{std::cbegin(vkExtension.extensionName),
									 std::find(
										 std::cbegin(vkExtension.extensionName),
										 std::cend(vkExtension.extensionName),
										 '\0')},
